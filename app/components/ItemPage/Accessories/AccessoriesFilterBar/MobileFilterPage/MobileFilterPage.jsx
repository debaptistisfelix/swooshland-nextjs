    ...
    return (
     ...
     `}>
       <section className={styles.container}>
        ...
        <h1 className={styles.label}>Filter by Category:</h1>
        <div className={styles.sortingOptionsList}>
              {CategoryFilterOptions.options.map((option)=>{
                  return <h1 onClick={()=>{
                    if(selectedCategory.length === 0){
                      /* handleBrandFilterOptionChage(option.label) */
                      handleMultipleCategoryFilterOptionChange(option.label)
                    } else if(selectedCategory.includes(option.label)){
                        removeCategoryFromMultipleFilter(option.label);
                    } else {
                      /* handleBrandFilterOptionChage(option.label) */
                      handleMultipleCategoryFilterOptionChange(option.label)
                    }
                  }} key={uuidv4()} className={`${styles.option} ${selectedCategory.includes(option.label) && styles.active}`}>{option.label}</h1>
              })}
          </div>
        </section>
        <section className={styles.filteringContainer}>
